...
def create_lp_constraints(model, ord=2, value=300, mode='initialization'):
    """Create L_p constraints for each layer, where p == ord, and value depends on mode (is radius, diameter, or factor to multiply average initialization norm with)"""
    constraints = []
...
class KNormBall(Constraint):
    """
    LMO class for the n-dim K-Norm-Ball = min( B_1(r), B_inf(r/K) )
    """
    ...
    def lmo(self, x):
        ...
        Returns 
        """
        super().lmo(x) # check validity of input
        # calculate lmo for B_1(r)
        b_1_lmo = torch.zeros_like(x)
        maxIdx = torch.argmax(torch.abs(x))
    ...
    def euclidean_project(self, x):
        """Projects x to the closest (i.e. in L2-norm) point on the LpBall (p = 1, 2, inf) with radius r."""
        super().euclidean_project(x)
        raise NotImplementedError(f"Projection not implemented for K-norm ball.")
